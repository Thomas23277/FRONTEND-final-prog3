...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 🛒 Comprar
    btnComprar.addEventListener("click", () => __awaiter(this, void 0, void 0, function* () {
        if (!carrito.length) {
        ...
        if (!user) {
            ...
            try {
                const res = yield fetch(`${API_URL}/api/usuarios/login?email=${email}&nombre=${encodeURIComponent(nombre)}`, {
                    method: "POST",
                });
                if (!res.ok)
        ...
        try {
            const response = yield fetch(`${API_URL}/api/pedidos`, {
                method: "POST",
